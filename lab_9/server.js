...
app.get('/team_stats', function(req,res){
  var total_games = 'select * from football_games;'; // Retrieves all the games from the football_games table
  var winning_games = 'select count(*) from football_games where visitor_score < home_score;'; // Home score is greater than visitor so increase wins
  var losing_games = 'select count(*) from football_games where visitor_score > home_score;'; // Home score is less than visitor so increase losses
  db.task('get-everything', task => {
      return task.batch([
  ...
  .then(data => {
    res.render('pages/team_stats',{
        ...
        games: data[0],
        total_wins: data[1][0].count,
        total_losses: data[2][0].count
      })
  })
  .catch(err => {
      // display error message in case an error
        ...
        data: '',
        total_wins: 'Error message',
        total_losses: 'Error message'
      })
    });
});
...
app.get('/player_info/get_player', function(req, res) {
  ...
  var picked_player = 'select * from football_players where id=' + player_id + ';';
  var games_played = 'select count(*) from football_games where ' + player_id + '=any(players);';

   db.task('get-everything', task => {
        return task.batch([
            task.any(players),
            task.any(picked_player),
